use grm_rs::{
    GraphClient, InMemoryBackend, NodeModel, NodePattern, Query, RelModel, Result, typed_id,
};
...
async fn main() -> Result<()> {
    ...
    {
        let client = GraphClient::new(InMemoryBackend::load_from_file(json_file)?);
        ...
        let mut tx = client.transaction().await?;
        let rels = tx
            .query_rel::<User, Authored>(
                Query::matching(NodePattern::<User>::new().out::<Authored>().to::<Post>())
                    .return_rel(),
            )
            .await?;
        for rel in rels {
    ...
    {
        let client = GraphClient::new(InMemoryBackend::load_from_binary_file(bin_file)?);
