...
- Import name: `grm_rs`
- Current Python version: `0.1.0a4`

Pip uses `==` for versions:

```bash
python -m pip install grm-rs==0.1.0a4
```
...
```bash
python -m pip install ./grm_rs-0.1.0a4-*.whl
```
...
- whether `0.1.0a4` has enough install smoke coverage across platforms
