...
- Import name: `grm_rs`
- Current Python version: `0.1.0a3`

Pip uses `==` for versions:

```bash
python -m pip install grm-rs==0.1.0a3
```
...
```bash
python -m pip install ./dist/grm_rs-0.1.0a3-*.whl
```
...
```bash
python -m pip install ./grm_rs-0.1.0a3-*.whl
```

Or, if they have access to the release asset URL:

```bash
python -m pip install "https://github.com/<owner>/<repo>/releases/download/grm-python-v0.1.0a3/<wheel-file>.whl"
```
...
- whether `0.1.0a3` has enough install smoke coverage across platforms
