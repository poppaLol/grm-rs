...
- Import name: `grm_rs`
- Current Python version: `0.1.0a2`

Pip uses `==` for versions:

```bash
python -m pip install grm-rs==0.1.0a2
```
...
```bash
python -m pip install ./grm_rs-0.1.0a2-*.whl
```
...
- whether `0.1.0a2` has enough install smoke coverage across platforms
