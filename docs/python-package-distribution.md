...
- Import name: `grm_rs`
- Current Python version: `0.1.0a5`

Pip uses `==` for versions:

```bash
python -m pip install grm-rs==0.1.0a5
```
...
```bash
python -m pip install ./grm_rs-0.1.0a5-*.whl
```
...
- whether `0.1.0a5` has enough install smoke coverage across platforms
